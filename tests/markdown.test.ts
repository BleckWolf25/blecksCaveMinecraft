...
describe('Markdown Compiler Util', () => {
  ...
  it('should compile links and images with lightbox attributes', () => {
    expect(compileMarkdown('[link text](https://google.com)')).toContain('<a href="https://google.com" target="_blank" rel="noopener">link text</a>');
    expect(compileMarkdown('![alt image](image.png)')).toContain('<img src="image.png" alt="alt image" data-lightbox="true"');
  });
  ...
  it('should compile code blocks with copy clipboards', () => {
    ...
    expect(compiled).toContain('<code>const a = 1;</code>');
  });
  ...
  it('should compile GitHubAlert callouts utilizing modular Alert elements', () => {
    ...
    expect(compileMarkdown('> [!IMPORTANT]\n> Critical task')).toContain('wiki-callout important-alert');
    expect(compileMarkdown('> [!WARNING]\n> High risk warnings')).toContain('wiki-callout warning-alert');
    expect(compileMarkdown('> sanguis et ferrum chronicles')).toContain('wiki-callout medieval-alert');
  });
